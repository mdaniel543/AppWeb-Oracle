...
BEGIN 
    ...
    IF (temp is null) THEN
        disponible := capital_total - sueldo;
    ELSE
        disponible := temp - sueldo;
    END IF;

    Update Departamento SET Capital_Disponible = disponible WHERE depaid = depa;
END;



CREATE OR REPLACE TRIGGER APLICANTE
BEFORE INSERT ON Aplicante
FOR EACH ROW
DECLARE 
 exist   EXCEPTION;
PRAGMA EXCEPTION_INIT (exist, -20001);
cont number;
BEGIN 
    SELECT Count(*) INTO cont FROM Aplicante WHERE cui = :new.cui;
